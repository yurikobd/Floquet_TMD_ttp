...
class ObservablesFF:
        ...
        if ax is not None:
            ...
            if cbar:
                if fig is None:
                    ...
                    cbar_ax = fig.colorbar(z1[0], ax=axs, pad=pad, shrink=0.8, aspect=15, fraction=0.05)
    ...
    def photoelAmpPlot(self, ax, Nmax=3, psi0=None, band_initial=0, cmap=None, vmax=0.25,
        ...
        if cmap is None:
            ...
            def create_gamma_colormap(base_cmap, gamma=0.5, name='gamma_adjusted_cmap'):
                """
                Adjusts the colors of a given colormap using a gamma transformation.

                Parameters:
                    base_cmap (str or Colormap): Base colormap (e.g., 'inferno').
                    gamma (float): Gamma value for adjustment.
                    name (str): Name of the new colormap.

                Returns:
                    Colormap: New colormap with gamma adjustment.
                """
                # Get the base colormap
                if isinstance(base_cmap, str):
                    base_cmap = mpl.cm.get_cmap(base_cmap)

                # Create the gamma-transformed colormap
                x = np.linspace(0, 1, 256)  # Linear space
                x_gamma = x ** gamma  # Apply gamma transformation
                colors = base_cmap(x_gamma)  # Map transformed values to the colormap

                # Create and return a new colormap
                return mpl.colors.ListedColormap(colors, name=name)


            cmap = create_gamma_colormap('inferno_r', gamma=gamma)
